 ...
 */
public class MyUserRoleFilter implements ActionFilter {
    ...
    public View match(ActionContext ac) {
        ...
        if (Role.ADMIN.value().equalsIgnoreCase(role) || Role.USER.value().equalsIgnoreCase(role)) {
            return null;
        } else {
            return new UTF8JsonView().setData(Result.error(555, "没有权限或登录超时,请重新登录"));
        }
    }
}
